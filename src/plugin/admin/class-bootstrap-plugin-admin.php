 ...
 * @subpackage Bootstrap_Plugin/admin
 */
...
class Bootstrap_Plugin_Admin
{
	...
	public function add_admin_menu()
	{
		...
		add_menu_page(
			...
			'manage_options',
			'custom-plugin',
			[$this, 'display_custom_plugin_admin_page'],
	...
	{
		require_once plugin_dir_path(__FILE__) . 'partials/bootstrap-plugin-admin-display.php';
	}
	...
	{
		add_settings_section('section_one', 'Section One', [$this, 'section_callback'], 'wordpress-custom-plugin-options');
	}
	...
	public function setup_fields()
	{
		$fields = [
			[
				...
				'supplemental' => '',
				'default'      => plugin_dir_url(dirname(__FILE__)) . 'public/img/image.png',
			],
			[
		...
		foreach ($fields as $field) {
			add_settings_field($field['uid'], $field['label'], [$this, 'field_callback'], 'wordpress-custom-plugin-options', $field['section'], $field);
	...
	public function field_callback($arguments)
	{
		...
		switch ($arguments['type']) {
			...
			case 'image':
				...
				$image_attributes = wp_get_attachment_image_src($value, [$width, $height]);
				...
				$markup .= '
				<img data-src="' . $image['src'] . '" src="' . $image['src'] . '" width="180px" height="auto" />';
				printf('<div class="upload">%s</div>', $markup);
	...
	public function enqueue_styles()
	{
         ...
         * defined in Bootstrap_Plugin_Loader as all of the hooks are defined
         * in that particular class.
         *
         * The Bootstrap_Plugin_Loader will then create the relationship
         * between the defined hooks and the functions defined in this
         * class.
         */

		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/custom-bootstrap-plugin-admin.css', [], $this->version, 'all');
	}
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script(
			$this->plugin_name,
			plugin_dir_url(__FILE__) . 'js/custom-bootstrap-plugin-admin.js',
			['jquery'],
			$this->version,
			false
		);
